 ...
 (function($, document, window, undefined) {
    ...
    Plugin.prototype = {
        ...
        _initDate: function() {
            ...
            for (var i = 0; i < this.options.calendars; i++) {
                this.currentDate[i] = this.currentDate[i] || new Date(date.getFullYear(), date.getMonth(), 1, 0, 0, 0, 0);
                if (this.mode === 'multiple') {
       ...
       /* Скрывает календарь */
        ...
        _buildDays: function(i) {
            ...
            for (var m = 0; m < 42; m++) {
                ...
                if (m < daysFromPrevMonth) {
                    ...
                    date_str_attr = 'data-id="' + this.options.calendar_id + '" data-date="' + dateArray[m].format("yyyymmdd") + '" onClick="select_date($(this).data(\'date\'), $(this).data(\'id\'));"';
                }
                ...
                html += '<span class="' + className + '" ' + date_str_attr + '>' + content + '<i class="price_col">'+2500+'</i>' +'</span>';
            }
            html += '</div></div>';
            return html;
        },
        _buildMonths: function(i) {
            ...
            for (var m = 0; m < 12; m++) {
                ...
                html += '<span class="month-' + m + ' ' + className + '" onClick="return false;">' + content[m] + '</span>'
            }
            html += '</div>';
            return html;
        },
                ...
                if (_target.length === 1) {
                    ...
                    switch (_target[0].className) {
                        ...
                        case this.namespace + '-prev':
                            this.prev(i);
                            refresh_calendar(this.options.calendar_id);
                            break;
        ...
        next: function(i, press) {
            ...
            switch (this.views[i]) {
                case 'days':
                    ...
                    break;
                case 'months':
                    date.setYear(this.currentYear[i] + 2);
                    if (press) {
                        this.focusDate[i] = new Date(date.getFullYear(), this.focusDate[i].getMonth(), this.focusDate[i].getDate(), 0, 0, 0, 0);
                    }
                    break;
